...
def get_video_url(movie_id, api_key):
    ...
    connection = requests.get(MOVIE_URL+str(movie_id)+"/videos?api_key="+api_key+LANG)
...
def get_movie_object_given_dict(movie_data_dict, api_key, img_url_start):
    """Takes the result movie dictionary and api key along with image url to start 
    and returns the movie object relate to dictionary."""
...
def get_movies_objects_list(search_url, api_key, query_append, img_url, query_list):
    """Connects to TMDB Api with given API key and search url and generates the movie data list."""
    movies = []
    for i in range(len(query_list)):
        connection = requests.get(search_url + api_key + query_append + movies_list[i])
        parsed_json = json.loads(connection.text)
        connection.close()
        if len(parsed_json['results']) == 0:
            print("Not found '" + " ".join(movies_list[i].split('+')) + "' movie..No Worries Skipping it.")
            continue
        result_dict = parsed_json['results'][0]
        movies.append(get_movie_object_given_dict(result_dict, api_key, img_url))
    return movies
...
if __name__ == '__main__':
    ...
    movies = get_movies_objects_list(SEARCH_URL, API_KEY, QUERY_APPEND, IMAGE_URL, movies_list)
